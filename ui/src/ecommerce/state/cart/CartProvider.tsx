...
/**
 * Returns true when an item has all required CartItem fields with valid values.
 * Applied to both localStorage and server-merged items to reject corrupt data.
 */
function isValidCartItem(item: unknown): item is CartItem {
  return (
    ...
    typeof (item as CartItem).titleSnapshot === 'string'
  );
}
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const mergeCustomerRef = React.useRef<string | null>(null);

  // Hydrate with schema validation to reject corrupted localStorage data
  React.useEffect(() => {
    const storage = getSafeStorage();
    if (!storage) return;
    const saved = readJson<CartState>(storage, storageKey);
    if (saved && Array.isArray(saved.items)) {
      const validItems = saved.items.filter(isValidCartItem);
      dispatch({
        type: 'cart/hydrate',
        state: {
          items: validItems,
          promoCode: typeof saved.promoCode === 'string' ? saved.promoCode : undefined,
        },
      });
    }
    setIsHydrated(true);
  ...
  // Persist (only after hydration to avoid overwriting saved cart with empty initial state)
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    if (mergeCustomerRef.current !== customerId) {
      hasMergedRef.current = false;
      lastSyncedRef.current = null;
      mergeCustomerRef.current = customerId;
    }
    ...
    (async () => {
      try {
        const merged = config.adapter.mergeCart
          ? await config.adapter.mergeCart({ customerId, cart: stateRef.current })
          : await config.adapter.getCart!({ customerId });

        if (cancelled) return;
        if (merged && Array.isArray(merged.items)) {
          hasMergedRef.current = true;
          const validMergedState = { ...merged, items: merged.items.filter(isValidCartItem) };
          dispatch({ type: 'cart/hydrate', state: validMergedState });
          lastSyncedRef.current = JSON.stringify(validMergedState);
        }
      } catch {
        if (!cancelled) {
          hasMergedRef.current = false;
  ...
  }, [config.adapter, customerId, isHydrated, isSignedIn]);
  ...
  React.useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      config.adapter
        ...
        .catch((err: unknown) => {
          console.warn('[CedrosPay] Cart server sync failed:', err instanceof Error ? err.message : err);
        });
    }, syncDebounceMs);
