...
export class CreditsManager implements ICreditsManager {
      ...
      return await this.circuitBreaker.execute(async () => {
        return await retryWithBackoff(
          async () => {
            ...
            getLogger().debug('[CreditsManager] Requesting quote');
      ...
      return await this.circuitBreaker.execute(async () => {
        return await retryWithBackoff(
          async () => {
            ...
            // 402 is expected for x402 protocol — it contains the quote
      ...
      return await this.circuitBreaker.execute(async () => {
        return await retryWithBackoff(
          async () => {
            ...
            getLogger().debug('[CreditsManager] Creating hold');
  ...
  async createCartHold(options: CreateCartCreditsHoldOptions): Promise<CreditsHoldResponse> {
    ...
    const cartHoldIdempotencyKey = generateUUID();
    try {
      return await this.circuitBreaker.execute(async () => {
        return await retryWithBackoff(
          async () => {
            const url = await this.routeDiscovery.buildUrl(`/paywall/v1/cart/${encodeURIComponent(cartId)}/credits/hold`);
            ...
            const response = await fetchWithTimeout(url, {
              ...
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`,
                'Idempotency-Key': cartHoldIdempotencyKey,
              },
              body: JSON.stringify({}),
            });
      ...
      return await this.circuitBreaker.execute(async () => {
        return await retryWithBackoff(
          async () => {
            ...
            getLogger().debug('[CreditsManager] Authorizing payment');
      ...
      return await this.circuitBreaker.execute(async () => {
        return await retryWithBackoff(
          async () => {
            ...
            const url = await this.routeDiscovery.buildUrl(`/paywall/v1/cart/${encodeURIComponent(cartId)}/credits/authorize`);
  ...
  async releaseHold(holdId: string, authToken: string): Promise<void> {
    ...
    try {
      await this.circuitBreaker.execute(async () => {
        const url = await this.routeDiscovery.buildUrl(`/paywall/v1/credits/hold/${encodeURIComponent(holdId)}/release`);
        const response = await fetchWithTimeout(url, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${authToken}`,
  ...
  async processPayment(
    ...
    try {
      ...
      // Release hold if authorization failed (authorizePayment returns {success:false}
      // instead of throwing on server errors, so the catch block won't handle cleanup)
      if (!result.success && holdId) {
        try {
          await this.releaseHold(holdId, authToken);
        } catch (releaseError) {
          getLogger().warn('[CreditsManager] Failed to release hold after auth failure:', releaseError);
        }
      }
