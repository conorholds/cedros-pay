...
export function useX402Payment() {
  ...
  // Refs to avoid stale closures in async payment flows.
  // If the wallet changes mid-flow, the ref reflects the latest value.
  ...
  // Uses a timestamp instead of boolean so stale locks auto-expire after 60s
  // (prevents user from getting stuck if wallet prompt hangs).
  const processingStartRef = useRef(0);
  const PROCESSING_TIMEOUT_MS = 60_000;

  /** Returns true if a payment is actively in-flight (auto-expires after 60s). */
  const isPaymentProcessing = () => {
    const start = processingStartRef.current;
    return start > 0 && Date.now() - start < PROCESSING_TIMEOUT_MS;
  };
  ...
  const executePaymentFlow = useCallback(
    async (
      ...
      // Snapshot wallet state from refs to avoid stale closures across async awaits.
      // If the wallet disconnects or changes mid-flow, we detect it before signing.
      const currentPublicKey = publicKeyRef.current;
      const currentSignTransaction = signTransactionRef.current;

      if (!currentPublicKey || !currentSignTransaction) {
      ...
      if (isGasless) {
        getLogger().debug('[useX402Payment] Gasless flow enabled');
        // Gasless flow: Backend builds transaction with compute budget, then co-signs and submits
        getLogger().debug('[useX402Payment] Building gasless transaction');
        const { transaction: txBase64, blockhash } = await x402Manager.buildGaslessTransaction({
          resourceId,
          userWallet: currentPublicKey.toString(),
          feePayer: requirement.extra!.feePayer,
          couponCode,
        });

        getLogger().debug('[useX402Payment] Deserializing backend transaction');
        const transaction = walletManager.deserializeTransaction(txBase64);

        // Validate wallet hasn't changed before requesting signature
        if (publicKeyRef.current?.toString() !== currentPublicKey.toString()) {
        ...
        const partialTx = await walletManager.partiallySignTransaction({
          transaction,
          signTransaction: currentSignTransaction,
          blockhash,
        });

        getLogger().debug('[useX402Payment] Submitting partial transaction');
        ...
        const transaction = await walletManager.buildTransaction({
          requirement,
          payerPublicKey: currentPublicKey,
        });
    ...
    [x402Manager, walletManager]
  );

  const processPayment = useCallback(
    async (resource: string, couponCode?: string, metadata?: Record<string, string>) => {
      // Deduplication: prevent concurrent payment requests (auto-expires after 60s)
      ...
      try {
        ...
        getLogger().debug('[useX402Payment] Received quote', {
          amount: currentRequirement.maxAmountRequired,
        });
