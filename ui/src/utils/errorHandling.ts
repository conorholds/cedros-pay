...
export async function parseErrorResponse(
  ...
  // Try JSON first; if that fails, fall back to text.
  // Use clone() so the body stream is available for the text fallback.
  // If clone() is unavailable (already consumed or mock), skip text fallback.
  const canClone = typeof response.clone === 'function';
  const cloned = canClone ? response.clone() : undefined;
  ...
  try {
    ...
    // JSON parsing failed — try to read body as text from the clone
    if (cloned) {
      try {
        const text = await cloned.text();
        if (text) return text;
      } catch {
        // text() also failed; use default
      }
    }
    // If we couldn't clone but the response has a text() method, try it directly.
    // This covers the case where json() threw because the response had no json() method
    // (e.g., plain-text error bodies) but text() is still available.
    if (!canClone && typeof response.text === 'function') {
      try {
        const text = await response.text();
        if (text) return text;
      } catch {
        // body already consumed or other error; use default
      }
    }
    return defaultMessage;
  }
