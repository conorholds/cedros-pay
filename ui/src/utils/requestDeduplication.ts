...
/**
 * Maximum number of entries retained in requestCache and buttonCooldowns.
...
export function setButtonCooldown(buttonId: string, cooldownMs: number = DEFAULT_COOLDOWN_MS): void {
  ...
  if (buttonCooldowns.size > MAX_CACHE_SIZE) {
    const keysToDelete = Array.from(buttonCooldowns.keys()).slice(0, buttonCooldowns.size - MAX_CACHE_SIZE);
...
export function markRequestProcessed(requestId: string): void {
  ...
  if (requestCache.size > MAX_CACHE_SIZE) {
    const keysToDelete = Array.from(requestCache.keys()).slice(0, requestCache.size - MAX_CACHE_SIZE);
    for (const key of keysToDelete) {
