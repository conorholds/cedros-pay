function x(y) {
  ...
  for (let t = 0; t < y.length; t++) {
    const e = y.charAt(t), o = e.charCodeAt(0);
    if (g[o] !== 255)
      throw new TypeError(e + " is ambiguous");
    g[o] = t;
  }
  const c = y.length, b = y.charAt(0), d = Math.log(c) / Math.log(256), U = Math.log(256) / Math.log(c);
  function E(t) {
    ...
    for (; r !== l && t[r] === 0; )
      r++, e++;
    const a = (l - r) * U + 1 >>> 0, n = new Uint8Array(a);
    for (; r !== l; ) {
      let f = t[r], i = 0;
      for (let w = a - 1; (f !== 0 || i < o) && w !== -1; w--, i++)
  ...
  function p(t) {
    ...
    for (; t[e] === b; )
      o++, e++;
    const l = (t.length - e) * d + 1 >>> 0, a = new Uint8Array(l);
    for (; e < t.length; ) {
      ...
      if (i === 255)
        return;
      let w = 0;
      for (let A = l - 1; (i !== 0 || w < r) && A !== -1; A--, w++)
        i += c * a[A] >>> 0, a[A] = i % 256 >>> 0, i = i / 256 >>> 0;
      if (i !== 0)
        throw new Error("Non-zero carry");
      r = w, e++;
    }
