...
exports.createMockCommerceAdapter = createMockCommerceAdapter;
function sleep(ms) {
...
function filterProducts(products, params) {
    ...
    if (params.category && params.category !== 'all') {
        ...
        if (cat)
            out = out.filter((p) => p.categoryIds.includes(cat.id));
    }
    if (params.search) {
    ...
    if (typeof priceMin === 'number')
        out = out.filter((p) => p.price >= priceMin);
...
function createMockCommerceAdapter() {
    return {
        ...
        async createSubscriptionCheckoutSession(payload) {
            ...
            if (payload.successUrl)
                return { kind: 'redirect', url: payload.successUrl };
