...
class WalletPool {
    constructor(poolId) {
        ...
        (0, logger_1.getLogger)().debug(`[WalletPool] Created pool: ${this.poolId}`);
    }
    /**
    ...
    getAdapters() {
        ...
        if (this.isCleanedUp) {
            (0, logger_1.getLogger)().warn(`[WalletPool] Attempted to use pool after cleanup: ${this.poolId}`);
            return [];
        }
        ...
        (0, logger_1.getLogger)().debug(`[WalletPool] Initializing adapters for pool: ${this.poolId}`);
        this.adapters = [
    ...
    async cleanup() {
        if (this.isCleanedUp) {
            (0, logger_1.getLogger)().debug(`[WalletPool] Pool already cleaned up: ${this.poolId}`);
            return;
        }
        (0, logger_1.getLogger)().debug(`[WalletPool] Cleaning up pool: ${this.poolId}`);
        ...
        const disconnectPromises = this.adapters.map(async (adapter) => {
            try {
                ...
                if (adapter.connected) {
                    (0, logger_1.getLogger)().debug(`[WalletPool] Disconnecting wallet: ${adapter.name}`);
                    await adapter.disconnect();
                }
            }
            catch (error) {
                // Don't throw on cleanup errors, just log them
                (0, logger_1.getLogger)().warn(`[WalletPool] Failed to disconnect wallet ${adapter.name}:`, error);
            }
        });
        await Promise.allSettled(disconnectPromises);
        // Clear adapter references
        this.adapters = null;
        (0, logger_1.getLogger)().debug(`[WalletPool] Pool cleanup complete: ${this.poolId}`);
    }
    /**
 ...
 * ```
 */
function createWalletPool(poolId) {
