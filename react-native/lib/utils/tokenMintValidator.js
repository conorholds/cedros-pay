...
exports.KNOWN_STABLECOINS = exports.STABLECOIN_METADATA = void 0;
exports.isKnownStablecoin = isKnownStablecoin;
exports.getStablecoinSymbol = getStablecoinSymbol;
exports.validateTokenMint = validateTokenMint;
exports.validateX402Asset = validateX402Asset;
/** Known stablecoins with full metadata */
exports.STABLECOIN_METADATA = {
...
exports.KNOWN_STABLECOINS = Object.fromEntries(Object.entries(exports.STABLECOIN_METADATA).map(([mint, meta]) => [mint, meta.symbol]));
/**
 * Type guard to check if a token mint is a known stablecoin
 */
function isKnownStablecoin(tokenMint) {
    return tokenMint in exports.KNOWN_STABLECOINS;
}
/**
 * Get the symbol for a known stablecoin mint address
 * Returns undefined if the mint is not recognized
 */
function getStablecoinSymbol(tokenMint) {
    if (isKnownStablecoin(tokenMint)) {
        return exports.KNOWN_STABLECOINS[tokenMint];
    }
    return undefined;
}
/**
 ...
 * ```
 */
function validateTokenMint(tokenMint, context = 'token mint', allowUnknown = false) {
    ...
    if (isKnownStablecoin(trimmedMint)) {
        const symbol = exports.KNOWN_STABLECOINS[trimmedMint];
        return {
            isValid: true,
    ...
    const knownMints = Object.entries(exports.KNOWN_STABLECOINS)
 ...
 */
function validateX402Asset(asset, resource = 'unknown', allowUnknown = false) {
    return validateTokenMint(asset, `X402Requirement (resource: ${resource})`, allowUnknown);
}
//# sourceMappingURL=tokenMintValidator.js.map