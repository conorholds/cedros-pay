...
async function parseErrorResponse(response) {
    ...
    try {
        ...
        if (body.error && typeof body.error.code === 'string') {
            ...
            return errors_1.PaymentError.fromErrorResponse(errorResponse, httpStatus);
        }
        // Unknown format - create generic error
        return new errors_1.PaymentError(errors_1.PaymentErrorCode.INTERNAL_ERROR, body.error || body.message || 'An unknown error occurred', false, undefined, httpStatus);
    }
    catch (parseError) {
        // Failed to parse JSON - use status text
        return new errors_1.PaymentError(errors_1.PaymentErrorCode.INTERNAL_ERROR, response.statusText || 'Request failed', false, undefined, httpStatus);
    }
}
/**
...
function isRetryableError(error) {
    return error instanceof errors_1.PaymentError && error.canRetry();
}
/**
