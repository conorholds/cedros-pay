 ...
 */
function StripeButton({ resource, items, successUrl, cancelUrl, metadata, customerEmail, couponCode, label, disabled = false, onAttempt, onSuccess, onError, style, textStyle, loadingColor = '#ffffff', }) {
    const { status, error, transactionId, processPayment, processCartCheckout } = (0, useStripeCheckout_1.useStripeCheckout)();
    ...
    const errorCode = error && typeof error !== 'string'
        ? error?.code ?? null
        : null;
    ...
    const executePayment = (0, react_1.useCallback)(async () => {
        (0, logger_1.getLogger)().debug('[StripeButton] executePayment with couponCode:', couponCode);
        const itemCount = isCartMode && items ? (0, cartHelpers_1.getCartItemCount)(items) : undefined;
        (0, eventEmitter_1.emitPaymentStart)('stripe', effectiveResource, itemCount);
        if (onAttempt) {
            onAttempt('stripe');
        }
        if (!isCartMode && !effectiveResource) {
            const errorMsg = 'Invalid payment configuration: missing resource or items';
            (0, logger_1.getLogger)().error('[StripeButton]', errorMsg);
            (0, eventEmitter_1.emitPaymentError)('stripe', errorMsg, effectiveResource, itemCount);
            if (onError) {
                onError(errorMsg);
            }
            return;
        ...
        let result;
        (0, eventEmitter_1.emitPaymentProcessing)('stripe', effectiveResource, itemCount);
        if (isCartMode && items) {
            (0, logger_1.getLogger)().debug('[StripeButton] Processing cart checkout with coupon:', couponCode);
            result = await processCartCheckout(items, successUrl, cancelUrl, metadata, customerEmail, couponCode);
        }
        else if (effectiveResource) {
            (0, logger_1.getLogger)().debug('[StripeButton] Processing single payment with coupon:', couponCode);
        ...
        if (result && result.success && result.transactionId) {
            (0, eventEmitter_1.emitPaymentSuccess)('stripe', result.transactionId, effectiveResource, itemCount);
            if (onSuccess) {
                onSuccess(result.transactionId);
            }
        }
        else if (result && !result.success && result.error) {
            (0, eventEmitter_1.emitPaymentError)('stripe', result.error, effectiveResource, itemCount);
            if (onError) {
    ...
    return (<react_native_1.View style={[styles.container, style]}>
      ...
      {transactionId && (<react_native_1.Text style={[styles.successText, { color: theme.tokens?.successText || '#22c55e' }]}>
...
//# sourceMappingURL=StripeButton.js.map