...
export function CedrosProvider({ config, children }) {
    ...
    const contextValue = useMemo(() => {
        const { stripeManager, x402Manager, walletManager, subscriptionManager, subscriptionChangeManager, creditsManager } = getOrCreateManagers(validatedConfig.stripePublicKey, validatedConfig.serverUrl ?? '', validatedConfig.solanaCluster, validatedConfig.solanaEndpoint, validatedConfig.dangerouslyAllowUnknownMint);
        return {
