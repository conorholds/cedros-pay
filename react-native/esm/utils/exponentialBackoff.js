...
export async function retryWithBackoff(fn, config = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (attempt > 0) {
                getLogger().debug(`[Retry:${name}] Succeeded on attempt ${attempt + 1}/${maxRetries + 1} after ${totalDelay}ms`);
            }
            return result;
        }
        catch (error) {
            ...
            if (isLastAttempt || !shouldRetryError) {
                getLogger().warn(`[Retry:${name}] Failed on attempt ${attempt + 1}/${maxRetries + 1}. ${isLastAttempt ? 'No more retries.' : 'Error not retryable.'}`);
                throw lastError;
            }
            // Calculate delay
            const delay = calculateDelay(attempt, initialDelayMs, backoffFactor, maxDelayMs, jitter);
            totalDelay += delay;
            getLogger().warn(`[Retry:${name}] Attempt ${attempt + 1}/${maxRetries + 1} failed: ${lastError.message}. Retrying in ${delay}ms...`);
            await sleep(delay);
        }
    }
