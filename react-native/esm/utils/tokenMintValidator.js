...
export const STABLECOIN_METADATA = {
...
export const KNOWN_STABLECOINS = Object.fromEntries(Object.entries(STABLECOIN_METADATA).map(([mint, meta]) => [mint, meta.symbol]));
/**
 * Type guard to check if a token mint is a known stablecoin
 */
export function isKnownStablecoin(tokenMint) {
 ...
 */
export function getStablecoinSymbol(tokenMint) {
 ...
 * ```
 */
export function validateTokenMint(tokenMint, context = 'token mint', allowUnknown = false) {
 ...
 */
export function validateX402Asset(asset, resource = 'unknown', allowUnknown = false) {
