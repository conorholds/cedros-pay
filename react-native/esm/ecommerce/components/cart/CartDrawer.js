...
export function CartDrawer({ trigger, side = 'right', open, onOpenChange, onCheckout, preferredTab, style, chatComponent, }) {
    ...
    return (<Sheet open={open} onOpenChange={onOpenChange}>
      ...
      <SheetContent side={side} style={[styles.sheetContent, style]}>
        ...
        <View style={styles.content}>
          {activeTab === 'chat' && chatComponent ? (<View style={styles.chatContainer}>
              {chatComponent}
            </View>) : (<CartPanel onCheckout={handleCheckout} style={styles.cartPanel}/>)}
        </View>
      </SheetContent>
    </Sheet>);
}
export function MiniCart({ onPress, style }) {
    const { config } = useCedrosShop();
    const cart = useCart();
    if (cart.items.length === 0) {
        return null;
    }
    return (<TouchableOpacity onPress={onPress} activeOpacity={0.7} style={[styles.miniCartContainer, style]}>
