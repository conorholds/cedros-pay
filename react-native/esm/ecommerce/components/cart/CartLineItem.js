...
function AlertIcon({ color = '#dc2626' }) {
    return (<View style={[styles.alertIcon, { borderColor: color }]}>
      <Text style={[styles.alertIconText, { color }]}>!</Text>
    </View>);
}
// Trash icon component for remove action
function TrashIcon({ color = '#dc2626' }) {
    return (<Text style={[styles.trashIcon, { color }]}>🗑</Text>);
}
export function CartLineItem({ item, onRemove, onSetQty, variant = 'table', style, inventory, }) {
    ...
    if (variant === 'compact') {
        return (<View style={[styles.compactContainer, style]}>
        {/* Product Image */}
        <View style={styles.compactImageContainer}>
          {item.imageSnapshot ? (<Image source={{ uri: item.imageSnapshot }} style={styles.compactImage} resizeMode="cover"/>) : (<View style={styles.compactImagePlaceholder}/>)}
        </View>
        ...
        <View style={styles.compactContent}>
          <View style={styles.compactRow}>
            ...
            <View style={styles.compactTextContainer}>
              ...
              {inventoryWarning && (<View style={styles.inventoryWarningRow}>
            ...
            <View style={styles.compactControlsContainer}>
              {isConfirmingRemove ? (<View style={styles.removeConfirmContainer}>
                  <Text style={styles.removeConfirmText}>Remove item?</Text>
                  <View style={styles.removeConfirmButtons}>
                    <Button variant="outline" size="sm" onPress={() => setIsConfirmingRemove(false)} style={styles.cancelButton}>
                      Cancel
                    </Button>
                    <Button variant="destructive" size="sm" onPress={onRemove} style={styles.confirmButton}>
                      Confirm
                    </Button>
                  </View>
                </View>) : (<View style={styles.quantityControls}>
                  ...
                  <Input keyboardType="numeric" value={String(item.qty)} onChangeText={(text) => {
                  ...
                  <Button size="sm" variant="outline" onPress={() => onSetQty(maxQty ? Math.min(maxQty, item.qty + 1) : item.qty + 1)} disabled={!canIncreaseQty} style={styles.qtyButton}>
                    +
                  </Button>
                </View>)}
            </View>
    ...
    return (<View style={[styles.tableContainer, style]}>
      {/* Image */}
      <View style={styles.tableImageContainer}>
        {item.imageSnapshot ? (<Image source={{ uri: item.imageSnapshot }} style={styles.tableImage} resizeMode="cover"/>) : (<View style={styles.tableImagePlaceholder}/>)}
      </View>
      ...
      <View style={styles.tableQtyContainer}>
        <View style={styles.quantityControls}>
          <Button size="sm" variant="outline" onPress={() => onSetQty(Math.max(1, item.qty - 1))} style={styles.qtyButton}>
            -
          </Button>
          <Input keyboardType="numeric" value={String(item.qty)} onChangeText={(text) => {
            const next = Math.floor(Number(text));
