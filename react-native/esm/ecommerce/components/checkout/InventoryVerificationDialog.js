...
export function InventoryVerificationDialog({ open, onOpenChange, issues, onRemoveItem, onUpdateQuantity, onGoToCart, style, }) {
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent style={[styles.content, style]}>
        ...
        <DialogFooter style={styles.footer}>
          {onGoToCart ? (<Button variant="outline" onPress={() => {
                onOpenChange(false);
                onGoToCart();
            }}>
              Go to Cart
...
function IssueRow({ issue, onRemove, onUpdateQty, }) {
    ...
    return (<View style={[styles.issueRow, isUnavailable && styles.issueRowUnavailable]}>
      <View style={styles.issueContent}>
        ...
        {issue.type === 'insufficient_stock' && issue.availableQty > 0 ? (<Text style={styles.issueDetail}>
            You requested {issue.requestedQty}, but only {issue.availableQty} available
          </Text>) : null}
      </View>

      <View style={styles.issueActions}>
        {issue.type === 'insufficient_stock' && issue.availableQty > 0 ? (<Button size="sm" variant="outline" onPress={() => onUpdateQty(issue.availableQty)} style={styles.updateButton}>
            Update to {issue.availableQty}
          </Button>) : null}
        <Button size="sm" variant="ghost" onPress={onRemove} textStyle={styles.removeButtonText}>
          Remove
        </Button>
      </View>
    </View>);
