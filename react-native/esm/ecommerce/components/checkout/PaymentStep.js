...
export function PaymentStep({ style, ctaLabel, renderEmbedded, embeddedFallback, renderCustom, customFallback, }) {
    ...
    return (<View style={[styles.container, style]}>
      {visibleMethods.length > 1 ? (<View style={styles.paymentMethodContainer}>
          <Text style={styles.paymentMethodLabel}>Payment method</Text>
          <Tabs value={methodId} onValueChange={setMethodId}>
            <TabsList style={styles.tabsList}>
              {visibleMethods.map((m) => (<TabsTrigger key={m.id} value={m.id} style={styles.tabTrigger}>
                  {m.label}
                </TabsTrigger>))}
            </TabsList>
          </Tabs>
      ...
      {checkout.error ? (<View style={styles.errorContainer}>
          <Text style={styles.errorText}>{checkout.error}</Text>
        </View>) : null}

      {custom ? (<View style={styles.sessionContainer}>
          {renderCustom ? (renderCustom(custom)) : (customFallback ?? (<View style={styles.fallbackContainer}>
                <Text style={styles.fallbackText}>
                  Checkout session created. Provide `renderCustom` to render a custom payment UI.
                </Text>
              </View>))}
        </View>) : embedded ? (<View style={styles.sessionContainer}>
          {renderEmbedded ? (renderEmbedded(embedded)) : (embeddedFallback ?? (<View style={styles.fallbackContainer}>
                <Text style={styles.fallbackText}>
                  Embedded checkout session created. Provide `renderEmbedded` to render your payment UI.
                </Text>
              </View>))}
        </View>) : (<Button style={styles.payButton} disabled={cart.items.length === 0 || isBusy || isCryptoUnavailable} loading={isBusy} onPress={() => {
                void handleCheckout(methodId);
            }}>
          {inventoryVerification.isVerifying ? 'Checking availability...' : isBusy ? 'Processing...' : label}
        </Button>)}

      {!embedded && !custom ? (<Text style={styles.hintText}>
          {isCryptoUnavailable
                ? 'Install a browser wallet to enable crypto payments.'
                : (hintText ?? 'You will be redirected to complete your payment.')}
        </Text>) : null}

      <InventoryVerificationDialog open={showInventoryDialog} onOpenChange={(open) => {
