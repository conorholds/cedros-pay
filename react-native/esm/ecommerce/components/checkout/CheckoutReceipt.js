...
export function CheckoutReceipt({ result, onContinueShopping, onViewOrders, style, }) {
    if (result.kind === 'idle')
        return null;
    if (result.kind === 'success') {
        ...
        <CardContent>
          ...
          {result.order ? (<View style={styles.orderCard}>
              ...
              <View style={styles.itemsContainer}>
                {result.order.items.slice(0, 4).map((it, idx) => (<View key={`${it.title}-${idx}`} style={styles.itemRow}>
                    <Text style={styles.itemTitle} numberOfLines={1}>
                      {it.title}
                    </Text>
                    <Text style={styles.itemQty}>Qty {it.qty}</Text>
                  </View>))}
                {result.order.items.length > 4 ? (<Text style={styles.moreItems}>
              ...
              {(result.order.receiptUrl || result.order.invoiceUrl) ? (<>
                  ...
                  <View style={styles.linksContainer}>
                    {result.order.receiptUrl ? (<TouchableOpacity>
                        <Text style={styles.link}>Receipt</Text>
                      </TouchableOpacity>) : null}
                    {result.order.invoiceUrl ? (<TouchableOpacity>
                        <Text style={styles.link}>Invoice</Text>
                      </TouchableOpacity>) : null}
                  </View>
                </>) : null}
            </View>) : result.orderId ? (<Text style={styles.sessionId}>
              Session/Order ID: <Text style={styles.mono}>{result.orderId}</Text>
            </Text>) : null}

          <View style={styles.buttonContainer}>
            {onContinueShopping ? (<Button onPress={onContinueShopping}>
                Continue shopping
              </Button>) : null}
            {onViewOrders ? (<Button variant="outline" onPress={onViewOrders}>
                View orders
              </Button>) : null}
          </View>
        </CardContent>
    ...
    if (result.kind === 'cancel') {
        ...
        <CardContent>
          ...
          {onContinueShopping ? (<View style={styles.singleButtonContainer}>
