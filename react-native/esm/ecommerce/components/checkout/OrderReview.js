...
export function OrderReview({ style }) {
    ...
    return (<View style={[styles.container, style]}>
      ...
      <View style={styles.itemsContainer}>
        {cart.items.map((it) => (<View key={`${it.productId}::${it.variantId ?? ''}`} style={styles.itemRow}>
            <View style={styles.itemLeft}>
              <View style={styles.itemImageContainer}>
                {it.imageSnapshot ? (<Image source={{ uri: it.imageSnapshot }} style={styles.itemImage}/>) : (<View style={styles.itemImagePlaceholder}/>)}
              </View>
      ...
      {showPromoCodes ? (<>
          <PromoCodeInput value={checkout.values.discountCode ?? cart.promoCode} onApply={(code) => {
