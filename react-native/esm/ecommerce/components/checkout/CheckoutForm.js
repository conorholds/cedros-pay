...
export function CheckoutForm({ style }) {
    const { config } = useCedrosShop();
    const checkout = useCheckout();
    const cart = useCart();
    const mode = config.checkout.mode;
    const req = React.useMemo(() => getCartCheckoutRequirements(cart.items, {
    ...
    return (<ScrollView style={[styles.container, style]} showsVerticalScrollIndicator={false}>
      {req.isDigitalOnly ? (<Card style={styles.digitalCard}>
          <CardContent>
      ...
      {req.hasPhysical && !(config.checkout.allowShipping ?? false) ? (<Card style={styles.errorCard}>
          <CardContent>
      ...
      {showContact ? (<ContactForm email={checkout.values.email} name={checkout.values.name} phone={checkout.values.phone} onEmailChange={(email) => checkout.setField('email', email)} onNameChange={(name) => checkout.setField('name', name)} onPhoneChange={(phone) => checkout.setField('phone', phone)} emailRequired={req.email === 'required'} nameRequired={req.name === 'required'} phoneRequired={req.phone === 'required'} emailError={checkout.fieldErrors.email} nameError={checkout.fieldErrors.name} phoneError={checkout.fieldErrors.phone}/>) : null}

      {wantsShipping ? (<AddressForm title="Shipping address" value={shippingAddress} onChange={(next) => checkout.setField('shippingAddress', next)} errors={{
      ...
      {mode === 'full' ? (<AddressForm title="Billing address" value={billingAddress} onChange={(next) => checkout.setField('billingAddress', next)}/>) : null}

      {config.checkout.allowTipping ? (<View style={styles.section}>
          <Text style={styles.sectionTitle}>Tip</Text>
          <View style={styles.fieldContainer}>
            <Label>Tip amount ({config.currency})</Label>
            <Input keyboardType="decimal-pad" value={String(checkout.values.tipAmount ?? 0)} onChangeText={(text) => checkout.setField('tipAmount', Number(text) || 0)}/>
          </View>
        </View>) : null}

      {mode === 'full' ? (<View style={styles.section}>
          <Text style={styles.sectionTitle}>Notes</Text>
          <View style={styles.fieldContainer}>
            <Label>Order notes (optional)</Label>
            <Input value={checkout.values.notes ?? ''} onChangeText={(text) => checkout.setField('notes', text)} placeholder="Delivery instructions, gift note..."/>
          </View>
        </View>) : null}
    </ScrollView>);
}
    ...
    return (<View style={[styles.section, style]}>
      ...
      <View style={styles.methodsContainer}>
        {methods.map((m) => {
            ...
            return (<Button key={m.id} variant={active ? 'default' : 'outline'} style={[styles.methodButton, active && styles.methodButtonActive]} onPress={() => onChange(m.id)}>
              <View style={styles.methodContent}>
                <View style={styles.methodLeft}>
                  <Text style={[styles.methodLabel, active && styles.methodLabelActive]}>
                    {m.label}
                  </Text>
                  {m.detail ? (<Text style={styles.methodDetail}>{m.detail}</Text>) : null}
                </View>
