...
export function ReceiptView({ order, storeName = 'Store', onDownload, style }) {
      ...
      <CardContent style={styles.content}>
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.itemsSection}>
            ...
            {order.items.map((item, idx) => (<View key={`${item.title}-${idx}`} style={styles.itemRow}>
          ...
          {/* Payment Info */}
          {order.source && (<View style={styles.paymentInfo}>
              <Text style={styles.paymentLabel}>
                Paid via {order.source === 'stripe' ? 'Card' : order.source === 'x402' ? 'Crypto' : 'Credits'}
              </Text>
              {order.purchaseId && (<Text style={styles.purchaseId} numberOfLines={1}>
          ...
          <View style={styles.actions}>
            {order.receiptUrl && (<Button variant="outline" onPress={() => handleOpenUrl(order.receiptUrl)}>
                View Online
              </Button>)}
            {onDownload && (<Button onPress={onDownload}>
                Download PDF
              </Button>)}
          </View>
        </ScrollView>
