...
export function OrderStatus({ status, showBadge = true, showLabel = false, style }) {
    const config = getStatusConfig(status);
    const capitalizedLabel = status.charAt(0).toUpperCase() + status.slice(1);
    return (<View style={[styles.container, style]}>
      {showBadge && (<Badge variant={config.variant}>
          {capitalizedLabel}
        </Badge>)}
      {showLabel && !showBadge && (<Text style={[styles.label, { color: config.color }]}>
          {capitalizedLabel}
        </Text>)}
    </View>);
}
