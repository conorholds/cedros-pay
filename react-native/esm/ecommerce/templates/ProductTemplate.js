...
export function ProductTemplate({ slug, style, onNavigateToCheckout, onNavigateToProduct, }) {
    const cart = useCart();
    ...
    return (<ScrollView style={[styles.container, style]} contentContainerStyle={styles.content}>
      {/* Breadcrumb */}
      ...
      <View style={styles.productLayout}>
        ...
        <View style={styles.detailsSection}>
          ...
          {isOutOfStock ? (<Text style={styles.outOfStock}>Out of stock</Text>) : typeof product.inventoryQuantity === 'number' && product.inventoryQuantity > 0 && product.inventoryQuantity <= 5 ? (<Text style={styles.lowStock}>
              Only <Text style={styles.lowStockBold}>{product.inventoryQuantity}</Text> left
            </Text>) : null}
          <Text style={styles.description}>{product.description}</Text>

          <VariantSelector product={product} value={{ selectedOptions: selected.selectedOptions, variantId: selected.variant?.id }} onChange={(next) => setSelected(next)}/>

          <View style={styles.actionRow}>
            <QuantitySelector qty={qty} onChange={setQty}/>
            <Button onPress={handleAddToCart} style={styles.addToCartButton} disabled={isOutOfStock}>
      ...
      {/* Related Products */}
      {storefrontSettings.sections.showRelatedProducts && relatedItems.length > 0 && (<View style={styles.relatedSection}>
          <Text style={styles.relatedTitle}>Related products</Text>
          <ProductGrid products={relatedItems} columns={{ base: 2, md: 4, lg: 4 }} layout={storefrontSettings.relatedProducts.layout.layout} imageCrop={storefrontSettings.relatedProducts.layout.imageCrop} onAddToCart={(p) => cart.addItem({
                productId: p.id,
                unitPrice: p.price,
                currency: p.currency,
