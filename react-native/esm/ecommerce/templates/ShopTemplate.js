...
export function ShopTemplate({ style, initialCategorySlug, onNavigateToProduct, onNavigateToCheckout, }) {
    ...
    return (<View style={[styles.container, style]}>
      ...
      <View style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <CartSidebar open={isCartOpen} onOpenChange={setIsCartOpen} onCheckout={() => onNavigateToCheckout?.()} trigger={<Button variant="outline" onPress={() => setIsCartOpen(true)}>
                Cart ({cart.count})
              </Button>}/>
        ...
        <View style={styles.layout}>
            ...
            <Card style={styles.sidebarCard}>
              ...
              <CardContent style={styles.sidebarContent}>
                <View style={styles.sidebarSection}>
                  ...
                  {catsLoading ? (<View style={styles.skeletonList}>
                      <Skeleton style={styles.skeletonItem}/>
                      <Skeleton style={styles.skeletonItem}/>
                      <Skeleton style={styles.skeletonItem}/>
                    </View>) : (<CategoryNav categories={categories} activeSlug={activeCategory} onSelect={onSelectCategory}/>)}
                </View>

                {sortOptions.length > 1 && (<View style={styles.sidebarSection}>
                    <Text style={styles.sectionTitle}>Sort</Text>
                    <View style={styles.sortOptions}>
                      {sortOptions.map((opt) => (<TouchableOpacity key={opt.value} style={[
                    styles.sortOption,
                ...
                <FilterPanel facets={facets} value={filters} onChange={(next) => {
            setFilters(next);
            setPage(1);
        }} onClear={() => {
            setFilters({});
          ...
          <View style={styles.main}>
            <View style={styles.mainHeader}>
              ...
              {storefrontSettings.shopPage.description && (<Text style={styles.pageDescription}>
                  {storefrontSettings.shopPage.description}
                </Text>)}
            </View>

            {error ? <ErrorState description={error}/> : null}
            {isLoading ? (<View style={styles.productGridSkeleton}>
                {Array.from({ length: 8 }).map((_, i) => (<Skeleton key={i} style={styles.productSkeleton}/>))}
              </View>) : (<View style={styles.productsSection}>
                <ProductGrid products={data?.items ?? []} columns={config.ui?.productGrid?.columns} onAddToCart={(p) => addToCart(p, null, 1)} onQuickView={(p) => {
                ...
                <View style={styles.pagination}>
                  ...
                  <Button variant="outline" disabled={!data?.hasNextPage} onPress={() => setPage((p) => p + 1)}>
                    Next
                  </Button>
                </View>
              </View>)}
      ...
      <QuickViewDialog product={quickViewProduct} open={isQuickViewOpen} onOpenChange={setIsQuickViewOpen} onAddToCart={(p, v, qty) => addToCart(p, v, qty)}/>
    </View>);
}
