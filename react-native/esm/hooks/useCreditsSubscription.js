...
export function useCreditsSubscription() {
     ...
     */
    const checkStatus = useCallback(async (resource, userId) => {
        setState((prev) => ({
            ...prev,
            status: 'checking',
            error: null,
     ...
     */
    const requestQuote = useCallback(async (resource, _interval, options) => {
        setState((prev) => ({
            ...prev,
            status: 'loading',
            error: null,
     ...
     */
    const processPayment = useCallback(async (resource, interval, authToken, options) => {
        if (!authToken) {
