...
export function useSubscription() {
     ...
     */
    const processSubscription = useCallback(async (request) => {
        setState((prev) => ({
            ...prev,
            status: 'loading',
            error: null,
     ...
     */
    const processMobileSubscription = useCallback(async (request) => {
        setState((prev) => ({
            ...prev,
            status: 'loading',
            error: null,
     ...
     */
    const checkStatus = useCallback(async (request) => {
        setState((prev) => ({
            ...prev,
            status: 'checking',
            error: null,
    ...
    const requestQuote = useCallback(async (resource, interval, options) => {
        ...
        try {
            const quote = await subscriptionManager.requestSubscriptionQuote(resource, interval, options);
            setState((prev) => ({
                ...prev,
                status: 'idle',
            }));
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to get subscription quote';
            setState((prev) => ({
                ...prev,
                status: 'error',
                error: errorMessage,
