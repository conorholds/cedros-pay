...
export function useCreditsPayment() {
     ...
     */
    const fetchQuote = useCallback(async (resource, couponCode) => {
        setState((prev) => ({
            ...prev,
            status: 'loading',
            error: null,
     ...
     */
    const fetchCartQuote = useCallback(async (items, couponCode) => {
        setState((prev) => ({
            ...prev,
            status: 'loading',
            error: null,
     ...
     */
    const processPayment = useCallback(async (resource, authToken, couponCode, metadata) => {
     ...
     * @param metadata - Optional metadata
     */
    const processCartPayment = useCallback(async (items, authToken, couponCode, metadata) => {
        // Deduplication: prevent concurrent payment requests
        if (isProcessingRef.current) {
            return { success: false, error: 'Payment already in progress' };
        }
