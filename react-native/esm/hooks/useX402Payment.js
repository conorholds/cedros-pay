...
export function useX402Payment() {
    ...
    const fetchQuote = useCallback(async (resource) => {
        try {
     ...
     */
    const executePaymentFlow = useCallback(async (requirement, resourceId, couponCode, metadata, resourceType = 'regular') => {
    ...
    const processPayment = useCallback(async (resource, couponCode, metadata) => {
    ...
    const processCartPayment = useCallback(async (items, metadata, couponCode) => {
        // Deduplication: prevent concurrent payment requests
        if (isProcessingRef.current) {
            return { success: false, error: 'Payment already in progress' };
        }
