...
export function useRefundVerification() {
     ...
     */
    const fetchRefundQuote = useCallback(async (refundId) => {
        try {
     ...
     */
    const processRefund = useCallback(async (refundId, couponCode) => {
     ...
     * @param refundId - Full refund ID including 'refund_' prefix
     */
    const processGaslessRefund = useCallback(async (refundId) => {
        if (!publicKey || !signTransaction) {
            throw new Error('Wallet not connected');
        }
        try {
