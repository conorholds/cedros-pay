 ...
 * - Theme integration
 */
export function ProductPrice({ amount, currency, originalAmount, size = 'medium', style, priceStyle, originalPriceStyle, }) {
 ...
 * ```
 */
export function PaymentMethodBadge({ product, paymentMethod, style, textStyle, }) {
    ...
    return (<View style={[
      ...
      <Text style={[badgeStyles.text, textStyle]}>
        {badgeText}
        {couponCode && (<Text style={badgeStyles.couponCode}> ({couponCode})</Text>)}
      </Text>
    </View>);
}
