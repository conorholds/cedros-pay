 ...
 */
export function CreditsButton({ resource, items, authToken, metadata, couponCode, label, disabled = false, onAttempt, onSuccess, onError, style, textStyle, loadingColor = '#ffffff', }) {
    ...
    return (<View style={[styles.container, style]}>
      ...
      {localizedError && (<Text style={[
                styles.errorText,
                { color: theme.tokens?.errorText || '#ef4444' },
            ]}>
          {localizedError}
        </Text>)}
      {transactionId && (<Text style={[
                styles.successText,
