...
import { emitPaymentStart, emitPaymentProcessing, emitPaymentSuccess, emitPaymentError, emitWalletConnect, emitWalletConnected, emitWalletError, } from '../utils/eventEmitter';
 ...
 */
export function CryptoSubscribeButton({ resource, interval, intervalDays, couponCode, label, disabled = false, onAttempt, onSuccess, onError, style, textStyle, loadingColor = '#ffffff', hideMessages = false, autoCheckStatus = true, }) {
    const { connected, connecting, connect, disconnect, select, wallets: availableWallets, wallet, publicKey, } = useWallet();
    const { status, error, subscriptionStatus, expiresAt, checkStatus, processPayment, } = useCryptoSubscription();
    ...
    const walletStateKey = useMemo(() => availableWallets.map((w) => `${w.adapter.name}-${w.readyState}`).join(','), [availableWallets]);
    const installedWallets = useMemo(() => availableWallets.filter(({ readyState }) => readyState === WalletReadyState.Installed || readyState === WalletReadyState.Loadable), [walletStateKey]);
    ...
    useEffect(() => {
        ...
        const attemptConnect = async () => {
            if (triggerConnect && wallet && !connected && !connecting) {
                getLogger().debug('[CryptoSubscribeButton] Wallet detected, attempting auto-connect:', wallet.adapter.name);
    ...
    const executeSubscriptionFlow = useCallback(async () => {
        ...
        if (!connected) {
            ...
            try {
                if (wallet) {
                    getLogger().debug('[CryptoSubscribeButton] Wallet already selected, connecting:', wallet.adapter.name);
                    emitWalletConnect(wallet.adapter.name);
                    await connect();
                }
                else {
    ...
    const handlePress = useMemo(() => createDedupedClickHandler(buttonId, executeSubscriptionFlow, {
        cooldownMs: 200,
    ...
    const handleSelectWallet = useCallback((walletName) => {
    ...
    return (<View style={[styles.container, style]}>
      <TouchableOpacity onPress={handlePress} disabled={isDisabled} style={[
            styles.button,
        ...
        {isProcessing ? (<ActivityIndicator color={loadingColor} size="small"/>) : (<Text style={[styles.buttonText, textStyle]}>{displayLabel}</Text>)}
      </TouchableOpacity>

      {/* Wallet Selector Modal */}
      <Modal visible={showWalletSelector && !hideMessages} transparent={true} animationType="fade" onRequestClose={() => setShowWalletSelector(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{t('wallet.select_wallet')}</Text>
              <TouchableOpacity onPress={() => setShowWalletSelector(false)} style={styles.closeButton} accessible={true} accessibilityRole="button" accessibilityLabel={t('ui.close')}>
                <Text style={styles.closeButtonText}>×</Text>
              </TouchableOpacity>
            </View>
            <ScrollView style={styles.walletList}>
              {installedWallets.map((w) => (<TouchableOpacity key={w.adapter.name} onPress={() => handleSelectWallet(w.adapter.name)} style={styles.walletOption} accessible={true} accessibilityRole="button" accessibilityLabel={w.adapter.name}>
                  <View style={styles.walletIcon}>
                    {w.adapter.icon && (<Image source={{ uri: w.adapter.icon }} style={styles.walletIconImage} resizeMode="contain"/>)}
                  </View>
      ...
      {/* Wallet controls */}
      {connected && !hideMessages && !showWalletSelector && (<View style={styles.walletControls}>
          ...
          <TouchableOpacity onPress={handleDisconnect} accessible={true} accessibilityRole="button">
            <Text style={styles.walletControlText}>{t('ui.disconnect')}</Text>
          </TouchableOpacity>
        </View>)}

      {/* Status messages */}
      {!hideMessages && localizedSolanaError && (<Text style={[styles.errorText, { color: theme.tokens?.errorText || '#ef4444' }]}>
          {localizedSolanaError}
        </Text>)}
      {!hideMessages && localizedError && (<Text style={[styles.errorText, { color: theme.tokens?.errorText || '#ef4444' }]}>
          {localizedError}
        </Text>)}
      {!hideMessages && isSubscribed && (<Text style={[styles.successText, { color: theme.tokens?.successText || '#22c55e' }]}>
