...
function ProrationPreview({ preview, onConfirm, onCancel, isLoading, }) {
    ...
    return (<View style={styles.prorationPreview}>
      ...
      <View style={styles.previewActions}>
        <TouchableOpacity onPress={onCancel} style={[styles.button, styles.cancelButton]} disabled={isLoading}>
          <Text style={styles.cancelButtonText}>Cancel</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={onConfirm} style={[styles.button, styles.confirmButton]} disabled={isLoading}>
          {isLoading ? (<ActivityIndicator color="#ffffff" size="small"/>) : (<Text style={styles.buttonText}>Confirm Change</Text>)}
        </TouchableOpacity>
      </View>
    </View>);
}
 ...
 */
export function SubscriptionManagementPanel({ resource, userId, availablePlans = [], onSubscriptionChanged, onSubscriptionCanceled, billingPortalReturnUrl, showBillingPortal = false, style, }) {
    const theme = useCedrosTheme();
    const { subscription, changePreview, status, error, loadSubscription, previewChange, changeSubscription, cancelSubscription, openBillingPortal, clearPreview, } = useSubscriptionManagement();
    // Load subscription on mount
    useEffect(() => {
        loadSubscription(resource, userId);
    }, [resource, userId, loadSubscription]);
    // Handle plan change preview
    const handlePreviewChange = useCallback(async (newResource, newInterval) => {
    ...
    // Handle cancellation
    const handleCancel = useCallback(async (immediate) => {
    ...
    return (<View style={[styles.container, style]}>
      {/* Error display */}
      {error && (<View style={[styles.errorContainer, { backgroundColor: theme.tokens?.errorBackground || '#fee2e2' }]}>
      ...
      {/* Loading state */}
      {isLoading && !subscription && (<View style={styles.loadingContainer}>
      ...
      {subscription && (<ScrollView style={styles.content}>
          <View style={styles.detailsSection}>
            ...
            {subscription.cancelAtPeriodEnd && (<View style={[styles.cancelNotice, { backgroundColor: theme.tokens?.errorBackground || '#fee2e2' }]}>
          ...
          {/* Available plans */}
          {availablePlans.length > 0 && !changePreview && (<View style={styles.plansSection}>
              ...
              {availablePlans.map((plan) => {
                    ...
                    return (<View key={plan.resource} style={[
                            styles.planCard,
                            isCurrent && [
                                styles.currentPlan,
                    ...
                    {plan.description && (<Text style={[styles.planDescription, { color: theme.tokens?.surfaceText || '#6b7280' }]}>
                        {plan.description}
                      </Text>)}
                    {isCurrent ? (<View style={[styles.currentBadge, { backgroundColor: theme.tokens?.successBackground || '#dcfce7' }]}>
                        <Text style={[styles.currentBadgeText, { color: theme.tokens?.successText || '#166534' }]}>
                          Current Plan
                        </Text>
                      </View>) : (<TouchableOpacity onPress={() => handlePreviewChange(plan.resource, plan.interval)} style={[
          ...
          <View style={styles.actions}>
            {showBillingPortal && subscription.paymentMethod === 'stripe' && (<TouchableOpacity onPress={handleOpenBillingPortal} style={[styles.portalButton, { backgroundColor: theme.tokens?.stripeBackground || '#635BFF' }]} disabled={isLoading}>
                <Text style={styles.portalButtonText}>Manage Billing</Text>
              </TouchableOpacity>)}
            {subscription.status === 'active' && !subscription.cancelAtPeriodEnd && (<TouchableOpacity onPress={() => handleCancel(false)} style={[styles.cancelSubscriptionButton, { borderColor: theme.tokens?.errorBorder || '#fca5a5' }]} disabled={isLoading}>
